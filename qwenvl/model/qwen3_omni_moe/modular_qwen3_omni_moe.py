...
from transformers.models.mimi.modeling_mimi import MimiLayerScale
...
from transformers.models.qwen2_5_omni.processing_qwen2_5_omni import Qwen2_5OmniProcessor, Qwen2_5OmniProcessorKwargs
    ...
    def get_rope_index(
        ...
        if input_ids is not None and (image_grid_thw is not None or video_grid_thw is not None):
            ...
            for i, input_ids in enumerate(total_input_ids):
                ...
                video_nums = ((vision_tokens == audio_start_token_id).sum() if use_audio_in_video else
                ...
                for _ in range(multimodal_nums):
                    ...
                    if (image_token_id in input_tokens or
                        video_token_id in input_tokens) and (remain_videos > 0 or remain_images > 0):
                    ...
                    # Audio in Video
                        ...
                        while (
                            video_data_index < video_llm_pos_ids.shape[-1] and
...
class Qwen3OmniMoeForConditionalGeneration(Qwen3OmniMoePreTrainedModel, GenerationMixin):
    ...
    def generate(
        ...
        **kwargs,
        ...
        thinker_embed = torch.cat([hidden_states[0] for hidden_states in thinker_result.hidden_states],
                                  dim=1).to(self.talker.device)  # [1 t d]
        ...
        multimodal_mask = ((thinker_result.sequences == self.config.thinker_config.audio_token_id) |
                           (thinker_result.sequences == self.config.thinker_config.image_token_id) |
                           (thinker_result.sequences == self.config.thinker_config.video_token_id)).to(
                               self.talker.device
                           )  # [1 t] # fmt: skip
        ...
        tts_bos_embed, tts_eos_embed, tts_pad_embed = (
            self.talker.text_projection(self.thinker.get_input_embeddings()(talker_special_tokens)).to(
                self.talker.device
            ).chunk(3, dim=1)
        )  # 3 * [1 1 d]
        ...
        talker_codes = (
            torch.stack([hid[-1] for hid in talker_result.hidden_states if hid[-1] is not None],
                        dim=1).transpose(1, 2).to(self.code2wav.device)
        )
            ...
            for _, special_token in positions:
                if special_token == self.audio_token:
                    ...
                    if not use_audio_in_video:
                        ...
                        while video_data_index < len(video_token_indices
