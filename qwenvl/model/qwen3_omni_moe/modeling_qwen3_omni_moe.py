...
class Qwen3OmniMoeVisionEncoder(Qwen3OmniMoePreTrainedModel):
    ...
    def __init__(self, config, *inputs, **kwargs) -> None:
        ...
        self.patch_embed = Qwen3OmniMoeVisionPatchEmbed(config=config, )
    ...
    def fast_pos_embed_interpolate(self, grid_thw):
        ...
        for pos_embed, t, h, w in zip(patch_pos_embeds, grid_ts, grid_hs, grid_ws):
            ...
            pos_embed = (
                pos_embed.view(t, h // merge_size, merge_size, w // merge_size, merge_size,
...
def load_balancing_loss_func(
    ...
    if attention_mask is None:
        ...
        tokens_per_expert = torch.sum(expert_mask.float() * expert_attention_mask,
                                      dim=0) / torch.sum(expert_attention_mask, dim=0)

        # Compute the mask that masks all padding tokens as 0 with the same shape of tokens_per_expert
        router_per_expert_attention_mask = (
            attention_mask[None, :, :, None].expand((num_hidden_layers, batch_size, sequence_length, num_experts)
                                                   ).reshape(-1, num_experts).to(compute_device)
        )

        # Compute the average probability of routing to these experts
        router_prob_per_expert = torch.sum(routing_weights * router_per_expert_attention_mask,
...
class SnakeBeta(nn.Module):
    ...
    def forward(self, hidden_states):
        ...
        hidden_states = hidden_states + (1.0 /
                                         (beta + self.no_div_by_zero)) * torch.pow(torch.sin(hidden_states * alpha), 2)
