...
class Qwen2_5OmniAudioEncoder(Qwen2_5OmniPreTrainedModel):
    ...
    def forward(
        ...
        padded_embed = padded_embed + self.positional_embedding.positional_embedding[:padded_embed.
                                                                                     shape[1], :].unsqueeze(0).to(
                                                                                         padded_embed.dtype
                                                                                     )
...
class Qwen2_5OmniThinkerTextModel(Qwen2_5OmniPreTrainedModel):
    ...
    def _update_causal_mask(
        ...
        if (
            ...
            attention_mask.device.type in ["cuda", "xpu", "npu"] and not output_attentions
        ):
...
class Qwen2_5OmniThinkerForConditionalGeneration(
    Qwen2_5OmniPreTrainedModelForConditionalGeneration, GenerationMixin, ModelTypeMixin
):
...
class AttentiveStatisticsPooling(nn.Module):
    ...
    def _length_to_mask(self, length, max_len=None, dtype=None, device=None):
        ...
        mask = torch.arange(max_len, device=length.device, dtype=length.dtype).expand(len(length),
...
class ECAPA_TimeDelayNet(torch.nn.Module):
    ...
    def __init__(self, config: Qwen2_5OmniDiTConfig):
        super().__init__()
        if len(config.enc_channels) != len(config.enc_kernel_sizes) or len(config.enc_channels
...
class Qwen2_5OmniForConditionalGeneration(Qwen2_5OmniPreTrainedModel, GenerationMixin):
    ...
    def generate(
        ...
        **kwargs,
        ...
        processed_thinker_hidden = ((embeds_to_talker, ) +
                                    thinker_result.hidden_states[0][1:], ) + thinker_result.hidden_states[1:]
        ...
        if "attention_mask" in kwargs:
            talker_attention_mask = torch.cat([kwargs["attention_mask"], kwargs["attention_mask"].new_ones((1, 2))],
                                              dim=1).to(self.talker.device)
