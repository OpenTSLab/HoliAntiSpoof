...
class Qwen2AudioPreTrainedModel(PreTrainedModel):
    ...
    def _init_weights(self, module):
        ...
        std = (
            ...
            if hasattr(self.config, "initializer_range") else self.config.audio_config.initializer_range
        )
...
class Qwen2AudioForConditionalGeneration(Qwen2AudioPreTrainedModel, GenerationMixin, ModelTypeMixin):
    def __init__(self, config: Qwen2AudioConfig):
        super().__init__(config)
    ...
    def _merge_input_ids_with_audio_features(
        ...
        audio_to_overwrite = torch.full((batch_size, max_token_num),
                                        True,
                                        dtype=torch.bool,
                                        device=inputs_embeds.device)
        ...
        if left_padding:
            ...
            val = (max_token_num -
                   seq_indices) <= (token_placeholder_num.sum(-1) - (attention_mask == 0).long().sum(-1))[:, None]
        else:
        ...
        **kwargs,
        ...
        if inputs_embeds is None:
            ...
            if input_features is not None and input_ids.shape[1] != 1:
                ...
                audio_attention_mask_ = padding_mask.view(batch_size, 1, 1,
                                                          max_seq_len).expand(batch_size, 1, max_seq_len, max_seq_len)
