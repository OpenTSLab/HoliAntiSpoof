...
from qwenvl.model.qwen2_5_vl.modeling_whisper import WhisperEncoder
from transformers import WhisperConfig, BertConfig
...
def apply_rotary_pos_emb_flashatt(q: torch.Tensor, k: torch.Tensor, cos: torch.Tensor,
...
def apply_rotary_pos_emb_vision(q: torch.Tensor, k: torch.Tensor, cos: torch.Tensor,
                                sin: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
...
class Qwen2_5_VLFlashAttention2(Qwen2_5_VLAttention):
    ...
    def forward(
        ...
        if (
            self.config.use_sliding_window and getattr(self.config, "sliding_window", None) is not None and
...
class Qwen2_5_VLModel(Qwen2_5_VLPreTrainedModel):
    ...
    def _update_causal_mask(
        ...
        if (
            self.config._attn_implementation == "sdpa" and not (using_static_cache or using_sliding_window_cache) and
        ...
        # DynamicCache or no cache
            ...
            target_length = (
                attention_mask.shape[-1] if isinstance(attention_mask, torch.Tensor) else past_seen_tokens +
                sequence_length + 1
            )
        ...
        if (
            self.config._attn_implementation == "sdpa" and attention_mask is not None and
            attention_mask.device.type in ["cuda", "xpu"] and not output_attentions
        ):
...
class Qwen2_5_VLForConditionalGeneration(Qwen2_5_VLPreTrainedModel, GenerationMixin, ModelTypeMixin):
    ...
    def __init__(self, config):
        ...
        self.audio = WhisperEncoder._from_config(config.audio_config, attn_implementation=config._attn_implementation)
        ...
        if getattr(config, "train_classify", False):
            self.classify_dim = config.classify_dim
            self.classify_type = config.classify_type if hasattr(config, "classify_type") else "last_layer"
            if self.classify_type == "last_layer":
                self.classify_linear = nn.Linear(config.hidden_size, self.classify_dim)
            elif self.classify_type == "last_layer_venc":
                self.classify_linear = nn.Sequential(
                    nn.Linear(config.hidden_size + config.vision_config.out_hidden_size, config.hidden_size),
                    nn.GELU(),
                    nn.Linear(config.hidden_size, self.classify_dim),
                )
            else:
                raise NotImplementedError
            print("Use Train Classify!")
    ...
    def get_rope_index(
        ...
        if input_ids is not None and (audio_lengths is not None and video_grid_thw is not None):
            # 处理音视频交织输入的情况
        ...
        elif input_ids is not None and (image_grid_thw is not None or video_grid_thw is not None):
            total_input_ids = input_ids
            if attention_mask is None:
                attention_mask = torch.ones_like(total_input_ids)
            position_ids = torch.ones(
    ...
    def get_video_embeds(
        self, pixel_values_videos, video_grid_thw, input_ids, inputs_embeds, original_video_embeds=None
    ):
        ...
        **loss_kwargs,
        ...
        if inputs_embeds is None:
            ...
            if pixel_values_videos is not None:
                inputs_embeds, video_embeds = self.get_video_embeds(
                    pixel_values_videos, video_grid_thw, input_ids, inputs_embeds
                )
            ...
            ) == 0 and (pixel_values is not None or pixel_values_videos is not None or audio_feature is not None):
        ...
        if train_classify:
            if self.classify_type == "last_layer":
                ...
                logits = self.classify_linear(hidden_states[:, -1, :])
            elif self.classify_type == "last_layer_venc":
                hidden_states = outputs[0][:, -1, :]
                mean_embeds = torch.mean(video_embeds, dim=0).unsqueeze(0)
                cat_embeds = torch.cat([hidden_states, mean_embeds], dim=-1)
                logits = self.classify_linear(cat_embeds)
            else:
                raise NotImplementedError
    ...
    def dpo_forward(
        ...
        **loss_kwargs,
        ...
        if pixel_values_videos is not None:
            chosen_embeds, video_embeds = self.get_video_embeds(
                pixel_values_videos, video_grid_thw, chosen_ids, chosen_embeds
            )
            reject_embeds = self.get_video_embeds(
                pixel_values_videos, video_grid_thw, reject_ids, reject_embeds, video_embeds
            )
        ...
        policy_chosen_hidden_states = policy_chosen_hidden_states[:, len_input:, :]
        policy_reject_hidden_states = policy_reject_hidden_states[:, len_input:, :]
        chosen_labels_in = chosen_labels[:, len_input:]
        reject_labels_in = reject_labels[:, len_input:]

        inputs_return = pad_and_stack(policy_chosen_hidden_states, policy_reject_hidden_states, 0.0)
        target_return = pad_and_stack(chosen_labels_in, reject_labels_in, -100)
        if train_type == "gdpo":
            inputs_embeds = self.model.embed_tokens(input_ids)
            if pixel_values is not None:
                inputs_embeds = self.get_image_embeds(
                    pixel_values, image_grid_thw, input_ids, inputs_embeds, image_embeds
                )

            if pixel_values_videos is not None:
                inputs_embeds = self.get_video_embeds(
                    pixel_values_videos, video_grid_thw, input_ids, inputs_embeds, video_embeds
                )
    ...
    def _expand_inputs_for_generation(
        ...
        **model_kwargs,
        ...
        def _expand_dict_for_generation(dict_to_expand):
            for key in dict_to_expand:
                if (
                    key != "cache_position" and dict_to_expand[key] is not None and
                    isinstance(dict_to_expand[key], torch.Tensor) and key not in visual_keys
                ):
